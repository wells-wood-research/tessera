...
def create_representatives_csv(clusters_file, input_csv, output_csv):
    ...
    clusters = pd.read_csv(
        ...
        sep="\t",
        header=None,
        names=["representative", "member", "similarity"],
    )
    ...
    representative_data = df.merge(
        clusters, left_on="UNIPROT", right_on="member", how="inner"
    )
    ...
    mmseqs_info = " | MMseqs2: Similarity Threshold 30%"
    header_info = f"{date_info} | {header}{mmseqs_info}\nUNIPROT,GO_ID,CHAIN,CLUSTER_REPRESENTATIVE,SIMILARITY\n"
    ...
    with open(output_csv, "w") as f:
        ...
        representative_data.to_csv(
            f,
            index=False,
            columns=["UNIPROT", "GO_ID", "CHAIN", "representative", "similarity"],
            header=False,
        )
...
def main():
    ...
    parser.add_argument(
        "--clusters_file", required=True, help="Cluster results file from MMseqs2"
    )
    parser.add_argument(
        "--input_csv", required=True, help="Input CSV file with original PDB data"
    )
    parser.add_argument(
        "--output_csv",
        default="uniprot_representative_pdbs.csv",
        help="Output CSV file for representative PDBs",
    )
